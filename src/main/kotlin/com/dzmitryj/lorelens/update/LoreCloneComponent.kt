...
import com.dzmitryj.lorelens.ffi.generated.FilterExcludeEvent
...
class LoreCloneComponent(
    ...
    private val directoryField = TextFieldWithBrowseButton().apply {
        addBrowseFolderListener(
            ...
            FileChooserDescriptorFactory.singleDir()
                .withTitle(LoreLensBundle.message("clone.directory.title")),
        )
    }
    private val sharedStore = JBCheckBox(LoreLensBundle.message("clone.shared.store"), true)
    ...
    private val viewFilterField = TextFieldWithBrowseButton().apply {
        addBrowseFolderListener(
            ...
            FileChooserDescriptorFactory.singleFile()
                .withTitle(LoreLensBundle.message("clone.view.title")),
        )
    }

    private val view = panel {
        row(LoreLensBundle.message("clone.url")) { cell(urlField).align(com.intellij.ui.dsl.builder.AlignX.FILL) }
        row(LoreLensBundle.message("clone.directory")) {
            cell(directoryField).align(com.intellij.ui.dsl.builder.AlignX.FILL)
        }
        row { cell(sharedStore) }
        row { comment(LoreLensBundle.message("clone.shared.store.comment")) }
        collapsibleGroup(LoreLensBundle.message("clone.advanced")) {
            row(LoreLensBundle.message("clone.view")) {
                cell(viewFilterField).align(com.intellij.ui.dsl.builder.AlignX.FILL)
            }
            row { comment(LoreLensBundle.message("clone.view.comment")) }
        }
    ...
    override fun doValidateAll(): List<ValidationInfo> = buildList {
        if (urlField.text.isBlank()) {
            add(ValidationInfo(LoreLensBundle.message("clone.url.required"), urlField))
        }
        if (directoryField.text.isBlank()) {
            add(ValidationInfo(LoreLensBundle.message("clone.directory.required"), directoryField))
        }
    }
    ...
    override fun doClone(listener: CheckoutProvider.Listener) {
        ...
        ProgressManager.getInstance().run(
            object : Task.Backgroundable(project, LoreLensBundle.message("clone.progress", url), true) {
                ...
                override fun onSuccess() {
                    ...
                    if (excluded.get() > 0) {
                        NotificationGroupManager.getInstance()
                            ...
                            .createNotification(
                                LoreLensBundle.message("clone.excluded.title"),
                                LoreLensBundle.message("clone.excluded", excluded.get()),
