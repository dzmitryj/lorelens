...
class LoreCreateRepositoryAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        ProgressManager.getInstance().run(
            object : Task.Backgroundable(project, LoreLensBundle.message("create.progress"), true) {
    ...
    private class CreateRepositoryDialog(project: Project) : DialogWrapper(project) {
        ...
        init {
            title = LoreLensBundle.message("create.title")
            init()
        }

        override fun createCenterPanel(): JComponent = panel {
            row {
                comment(LoreLensBundle.message("create.centralized.notice"))
            }
            row(LoreLensBundle.message("create.url")) { cell(urlField).align(AlignX.FILL) }
            row(LoreLensBundle.message("clone.directory")) { cell(directoryField).align(AlignX.FILL) }
            row { cell(sharedStore) }
            row { comment(LoreLensBundle.message("clone.shared.store.comment")) }
        }

        override fun doValidate(): ValidationInfo? = when {
            url.isEmpty() -> ValidationInfo(LoreLensBundle.message("clone.url.required"), urlField)
            directory.isEmpty() ->
                ValidationInfo(LoreLensBundle.message("clone.directory.required"), directoryField)
            else -> null
        }
    }
}
